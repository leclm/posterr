...
const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose, user }) => {
  ...
  const handleFollowToggle = () => {
    ...
    if (isFollowing) {
      loggedUser.following = loggedUser.following.filter((u: User) => u.id !== user.id);
      targetUser.followers = targetUser.followers.filter((u: User) => u.id !== loggedUserId);
      setFollowersCount((prev) => prev - 1);
    } else {
      loggedUser.following.push({ id: user.id, username: user.username, nick: user.nick });
      targetUser.followers.push({ id: loggedUserId, username: loggedUser.username, nick: loggedUser.nick });
      setFollowersCount((prev) => prev + 1);
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="w-full max-w-4xl h-[90vh] bg-gray-800 rounded-lg p-6 shadow-lg overflow-hidden">
            ...
            <div className="flex justify-between mt-4 text-gray-300">
              ...
              <p>
                <span className="font-semibold">{user.posts.length || 0}</span> Posts
              </p>
            </div>
